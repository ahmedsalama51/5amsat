...
class PostControllers extends Controller
{
    ...
    public function show($id,$title){
		...
		if(sizeof($post) > 0)
		{
			...
			return view ('pages.single',compact('permission','post_tags','post','user','category'),['posts'=> $this->posts,'categories'=> $this->categories,'tags'=> $this->tags,'topviewd'=>$this->topviewd]);
		}
		else	
	...
	public function store(Request $request){
		...
		$validator = Validator::make($reqArr,[
			'title' => 'requierd|max:255|unique:posts',
			'description' => 'requierd|max:100',
			'content' => 'requierd|min:6',
			'image' => 'requierd',
			'posted_date' => 'requierd',
			'user_id' => 'requierd',
		...
		{// validator dosn't work
			...
			return view ('pages.addPost',compact('massage'),['posts'=> $this->posts,'categories'=> $this->categories,'tags'=> $this->tags,'topviewd'=>$this->topviewd]);
		}
		else
		{
			$user = Auth::user();
	...
	public function append($id,Request $request){
		...
		{// validator dosn't work
			$messages = $validator->messages();
			return view ('pages.editPost',compact('massage'),['posts'=> $this->posts,'categories'=> $this->categories,'tags'=> $this->tags,'topviewd'=>$this->topviewd]);
		}
		else
		{
	...
	public static function is_editedable($id){
		...
		if(sizeof($post) > 0)
		{
			if(Auth::user()->id == $post->user_id)// the author user is the currunt loged user
			{
				$permission = 1;
			}
